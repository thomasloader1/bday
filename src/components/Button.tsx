...
const Button: FC<ButtonProps> = ({type='button', loading,text, onClick, className, color, isLink, disabled}) => {
  ...
  const getColor = (color:string) => {
    const colorMap: any = {
      ...
      'orange': 'bg-orange-700 hover:bg-orange-800 focus:ring-4 focus:ring-orange-300 dark:bg-orange-600 dark:hover:bg-orange-700 dark:focus:ring-orange-800',
  };
  ...
  return (
    isLink?.href ? <Link href={isLink.href} className={`${getColor(color || "")} ${className || ""} focus:outline-none text-white  font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2`}>{text}</Link> :
    <button disabled={disabled} type={type} onClick={onClick} className={`${getColor(color || "")} ${className || ""} focus:outline-none text-white font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 disabled:cursor-not-allowed disabled:bg-gray-500 disabled:hover:bg-gray-500`}>
        {loading ? (
